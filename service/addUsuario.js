...
async function addUsuario(userInfo) {
    const uuid = uuidv4();
    ...
    const cuidadores = [userInfo.cuidador]
    const cuidadores_uuid = [userInfo.cuidador_uuid]
...
async function getUser(email) {
    ...
    return await dynamodb.scan(params).promise().then(response => {
        console.log("Comprobar si usuario existe");
